...
contract SetPermissions is Script {
    function grantRole(address token, address pool) public {
        vm.startBroadcast();
        IRebaseToken(token).granMintAndBurnRole(pool);
    }
    ...
    function setAdmin(address token, address pool) public {
        ...
        TokenAdminRegistry(networkDetails.tokenAdminRegistryAddress).setPool(address(token), pool);
        vm.stopBroadcast();
    }
}
...
contract VaultDeployer is Script {
    function run(address _rebaseToken) public returns (Vault vault) {
        ...
        IRebaseToken(_rebaseToken).granMintAndBurnRole(address(vault));
        vm.stopBroadcast();
    }
}
